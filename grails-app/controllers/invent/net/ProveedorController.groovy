...
class ProveedorController {
    ...
    def list = {
        ...
        if( params.q ){            
            listresp = Proveedor.search(params.q + "*", [max:100]).results
            countresp = listresp.size()
        }else{  
            params.max = Math.min(params.max ? params.int('max') : 15, 100)
            //[proveedorInstanceList: Proveedor.list(params), proveedorInstanceTotal: Proveedor.count()]
            listresp = Proveedor.list(params)
            countresp =  Proveedor.count()
        }
        
        [proveedorInstanceList: listresp, proveedorInstanceTotal: countresp]
    }
